...
import fun.aegis.utils.features.aura.render.AimbotOutlineEffect;
...
public class AimBot extends Module {
     ...
     BooleanSetting targetEspEnabled = new BooleanSetting("Target ESP", "Отображать ESP для текущей цели")
               .setValue(true);

     SelectSetting targetEspType = new SelectSetting("ESP Type", "Тип отображения цели")
               .value("Circle", "Cube", "Ghosts")
               .selected("Circle")
               .visible(() -> targetEspEnabled.isValue());

     BooleanSetting outlineEffectEnabled = new BooleanSetting("Outline Effect", "Красивый аутлайн вокруг цели")
               .setValue(true);
     ...
     public AimBot() {
          ...
          setup(range, fov, wallCheck, priority, aimPoints, smooth, speedH, speedV, silent, flickDelay, organic,
                    fovCircle, targets, targetEspEnabled, targetEspType, outlineEffectEnabled);
     }

     private double getTargetValue(LivingEntity entity) {
          double value = switch (priority.getSelected()) {
     ...
     private Vec3d getAimPos(LivingEntity entity) {
          ...
          double predictTicks = 2.0;
          Vec3d predictedPos = basePos.add(velocity.multiply(predictTicks));
          ...
          if (organic.isValue() || silent.isValue()) {
               ...
               return predictedPos.add(offsetX, offsetY, offsetZ);
          }
     ...
     public void onMotion(MotionEvent e) {
          ...
          // Обновляем lastTarget для Target ESP
          if (target != null) {
               lastTarget = target;
          }
     }
     ...
     public void onWorldRender(WorldRenderEvent e) {
          if (!targetEspEnabled.isValue() || lastTarget == null || !lastTarget.isAlive())
               return;
          ...
          switch (targetEspType.getSelected()) {
               ...
               case "Cube" -> Render3D.drawCube(lastTarget, anim, red, "1");
               case "Ghosts" -> Render3D.drawGhosts(lastTarget, anim, red, 0.62F);
               default -> {} // Другие типы требуют дополнительной реализации
          }
          
          // Рендеринг эффекта аутлайна
          if (outlineEffectEnabled.isValue()) {
               AimbotOutlineEffect.renderFullEffect(e.getStack(), lastTarget);
          }
     }
     ...
     private void handleRotation(Turns targetAngle, float smoothVal, float hSpeed, float vSpeed) {
          ...
          // Улучшенная интерполяция с ускорением
          float yawAccel = Math.min(Math.abs(yawChange) * 1.2f, hSpeed);
          float pitchAccel = Math.min(Math.abs(pitchChange) * 1.2f, vSpeed);

          yawChange = Math.copySign(yawAccel, yawChange);
          pitchChange = Math.copySign(pitchAccel, pitchChange);
